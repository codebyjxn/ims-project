...
export class OrganizerController {
    ...
    try {
      ...
      if (currentDb === 'mongodb') {
        ...
        for (const concert of concerts) {
          ...
          `;
          const artistResult = await pool.query(artistQuery, [concert.concert_id]);
  ...
  async getOrganizerStats(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (currentDb === 'mongodb') {
        ...
        `;
        const totalConcertsResult = await pool.query(totalConcertsQuery, [organizerId]);
        ...
        `;
        const upcomingConcertsResult = await pool.query(upcomingConcertsQuery, [organizerId]);
  ...
  async createConcert(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (currentDb === 'mongodb') {
        ...
        const organizerCheck = await pool.query('SELECT user_id FROM organizers WHERE user_id = $1', [organizerId]);
        ...
        const arenaCheck = await pool.query('SELECT arena_id FROM arenas WHERE arena_id = $1', [arenaId]);
        ...
        for (const artistId of artists) {
          const artistCheck = await pool.query('SELECT artist_id FROM artists WHERE artist_id = $1', [artistId]);
        ...
        for (const zone of zones as ZoneData[]) {
          const zoneCheck = await pool.query(
            'SELECT zone_name FROM zones WHERE arena_id = $1 AND zone_name = $2', 
            [arenaId, zone.zone_name]
          );
        ...
        try {
          ...
          `;
          await pool.query(concertQuery, [concertId, organizerId, date, time, description, arenaId]);
          ...
          for (const zone of zones as ZoneData[]) {
            const zonePricingQuery = `
              ...
              VALUES ($1, $2, $3, $4)
            `;
            await pool.query(zonePricingQuery, [concertId, arenaId, zone.zone_name, zone.price]);
          }
    ...
    try {
      ...
      if (currentDb === 'mongodb') {
        ...
        try {
          ...
          await pool.query('DELETE FROM tickets WHERE concert_id = $1', [concertId]);
          await pool.query('DELETE FROM concert_zone_pricing WHERE concert_id = $1', [concertId]);
          await pool.query('DELETE FROM concert_features_artists WHERE concert_id = $1', [concertId]);
          
          // Delete the concert
          const result = await pool.query('DELETE FROM concerts WHERE concert_id = $1 RETURNING concert_id', [concertId]);
  ...
  async getConcertAnalytics(req: Request, res: Response): Promise<void> {
    try {
      const { concertId } = req.params;
      const currentDb = migrationStatus.getDatabaseType();
